...
For ready-to-build implementations of the common cases — button press, dropdown, tooltip, modal, drawer, toast, accordion, stagger, hold-to-confirm, tab indicator, scroll reveal, drag-to-dismiss — use companion doc `RECIPES.md` when it is available in the skill bundle. If it is not bundled, derive the implementation from the Build Sequence and **Never Ship** tables in this file rather than starting from a blank file.
