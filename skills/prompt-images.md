...
This skill describes general prompting techniques. To choose a model, query the host platform's model API or catalog—not memory of model names. On Replicate, search the API for current image models. The landscape changes weekly. Don't assume specific models exist or are still state-of-the-art based on names you've seen before. Always search for current options, then read the schema before running anything.

For pricing and feature comparison, use the platform's comparison docs or tools when available (on Replicate, use compare-models guidance if that skill is installed in your agent environment).
...
9. **Wrong model for the task**: Not every model is good at every task. Some excel at text rendering but struggle with object removal. Some are great at style transfer but poor at background editing. If a model struggles with a specific edit type, try a different one rather than fighting the prompt. Use the platform's model comparison docs when choosing.
